...
class AccountTestCase(TestCase):
    ...
    def tearDown(self):
        sqlite3Operations.run_query(
            account.accountDatabaseName, "DROP TABLE ACCOUNTS", commit=True
        )
    ...
    def test_login(self):
        ...
        self.assertEquals(
            self.tokenGenerator("testuser1", "testpassword1"),
            self.getJRvalue(jr1, "token"),
        )
        jr2 = account.login("testuser2", "testpassword2")
        self.assertEquals(
            self.tokenGenerator("testuser2", "testpassword2"),
            self.getJRvalue(jr2, "token"),
        )
        jr3 = account.login("testuser1", "testpassword1")
        self.assertEquals(
            self.tokenGenerator("testuser1", "testpassword1"),
            self.getJRvalue(jr3, "token"),
        )
        pass
    ...
    def test_login_failed(self):
        ...
        self.assertEquals(
            "Incorrect Password Or Username", self.getJRvalue(jr1, "error")
        )
        jr2 = account.login("testuser2", "testpassword2")
        self.assertEquals(
            self.tokenGenerator("testuser2", "testpassword2"),
        ...
        self.assertEquals(
            "Incorrect Password Or Username", self.getJRvalue(jr3, "error")
        )
        pass
